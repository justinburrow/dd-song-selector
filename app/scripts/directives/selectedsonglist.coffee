  ...
  .directive 'selectedSongList', ->
    ...
    link: (scope, element, attrs) ->
      angular.element('html').click (event) ->
        if angular.element(event.target).hasClass('mb-review') && !angular.element('.filter-col').hasClass('expand') || angular.element(event.target).hasClass('dt-review')
          angular.element('.selected-songs').toggleClass('expand')
          return
        if angular.element(event.target).hasClass('mb-options') && !angular.element('.selected-songs').hasClass('expand')
